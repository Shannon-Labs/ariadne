...
class BenchmarkRunner:
    ...
    def _generate_comparison_report(
        ...
        if summary_stats:
            ...
            fastest_backend = min(
                summary_stats.keys(),
                key=lambda k: summary_stats[k]["avg_execution_time"]
            )
            ...
            highest_throughput = max(
                summary_stats.keys(),
                key=lambda k: summary_stats[k]["max_throughput"]
            )
            ...
            most_efficient = min(
                summary_stats.keys(),
                key=lambda k: summary_stats[k]["avg_memory_peak"]
            )
            ...
            for platform, data in platform_comparison.items():
                ...
                if platform_values:
                    ...
                    if best_value is None or (
                        (metric in ["avg_execution_time", "avg_memory_peak"] and platform_best < best_value) or
                        (metric == "max_throughput" and platform_best > best_value)
                    ):
