    ...
    def can_simulate(self, circuit: QuantumCircuit) -> tuple[bool, str]:
        ...
        try:
            ...
            if estimated_memory > available_memory * 0.8:  # Don't use more than 80% of available memory
                return False, f"Insufficient memory: need {estimated_memory:.0f}MB, have {available_memory:.0f}MB"
        except ImportError:
        ...
        for instruction, _, _ in circuit.data:
            gate_name = instruction.name.lower()
            if gate_name not in ['id', 'x', 'y', 'z', 'h', 's', 'sdg', 't', 'tdg', 'sx', 'sxdg',
                               'rx', 'ry', 'rz', 'p', 'u1', 'u2', 'u3',
                               'cx', 'cy', 'cz', 'swap', 'ccx',
                               'measure', 'barrier', 'delay']:
...
def create_qulacs_backend(use_gpu: bool = True, gpu_device: int = 0, 
                         allow_cpu_fallback: bool = True) -> QulacsBackend:
    """
    ...
    """
    return QulacsBackend(use_gpu=use_gpu, gpu_device=gpu_device, 
