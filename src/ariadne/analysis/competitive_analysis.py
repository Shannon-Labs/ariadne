...
class CompetitiveAnalyzer:
    ...
    def _analyze_performance_results(self, circuit_name: str, circuit: QuantumCircuit,
        ...
        # Rank all frameworks
        ranked_frameworks = sorted(overall_scores.keys(), 
                                 key=lambda k: overall_scores[k], reverse=True)
    ...
    def generate_competitive_report(self, results: List[CompetitiveAnalysisResult]) -> str:
        ...
        report_lines.append(f"- **Average Ranking**: {avg_rank:.1f} out of {results[0].total_competitors if results else 0}")
        report_lines.append("")
