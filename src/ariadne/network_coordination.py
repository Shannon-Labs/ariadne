...
class TimePrecisionManager:
    ...
    def calibrate_with_reference(self, reference_timestamps: list[tuple[float, float]]):
        ...
        if len(self._calibration_data) > 0:
            ...
            time_diff = reference_timestamps[-1][0] - self._calibration_data[-2] if len(self._calibration_data) > 1 else 1.0
...
class NetworkCoordinator:
    ...
    def _heartbeat_loop(self):
        ...
        while self._running:
            try:
                ...
                # Update statistics
                self.stats['nodes_online'] = sum(1 for node in self.nodes.values() 
                                                if node.status == NetworkStatus.ONLINE)
                
                # Calculate average latency
                online_nodes = [node for node in self.nodes.values() 
                              if node.status == NetworkStatus.ONLINE]
                if online_nodes:
    ...
    async def _rebalance_load(self):
        ...
        underloaded_nodes = [node for node in self.nodes.values()
        ...
        if overloaded_nodes and underloaded_nodes:
            logger.info(f"Load imbalance detected: {len(overloaded_nodes)} overloaded, "
