...
class ObjectiveWeight:
    ...
    def normalize(self) -> ObjectiveWeight:
        ...
        total = (self.time_weight + self.memory_weight + self.energy_weight + 
                self.cost_weight + self.accuracy_weight + self.success_rate_weight)
...
class ParetoOptimizer:
    ...
    def _dominates(self, solution_a: BackendObjectiveScores, solution_b: BackendObjectiveScores) -> bool:
        ...
        for objective in objectives:
            ...
            if objective in [OptimizationObjective.MINIMIZE_TIME, OptimizationObjective.MINIMIZE_MEMORY,
                           OptimizationObjective.MINIMIZE_ENERGY, OptimizationObjective.MINIMIZE_COST]:
...
class ObjectiveScorer:
    ...
    def score_backend_objectives(self, circuit: QuantumCircuit, backend: BackendType,
                                context: UserContext) -> BackendObjectiveScores:
    ...
    def _score_energy_consumption(self, backend: BackendType, analysis: dict[str, Any],
        ...
        if context.hardware_profile.apple_silicon and backend in [BackendType.JAX_METAL, BackendType.QISKIT]:
            base_score *= 1.2
