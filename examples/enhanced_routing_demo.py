...
try:
    ...
    from ariadne.route.context_detection import detect_user_context, ContextDetector
...
def demonstrate_context_detection():
    ...
    print(f"🎯 Performance Preferences:")
...
def demonstrate_enhanced_routing(context):
    ...
    for circuit_name, circuit in circuits.items():
        ...
        for strategy in strategies:
            ...
            print(f"  {strategy.value:>15}: {decision.recommended_backend.value} "
...
def demonstrate_multi_objective_optimization(context):
    ...
    weight_configs = [
        ...
        ("Accuracy-Focused", ObjectiveWeight(accuracy_weight=0.6, time_weight=0.2, memory_weight=0.2)),
        ("Balanced", ObjectiveWeight(time_weight=0.3, accuracy_weight=0.3, memory_weight=0.2, energy_weight=0.2))
    ]
    ...
    for circuit_name, circuit in circuits.items():
        ...
        for config_name, weights in weight_configs:
            try:
                ...
                print(f"  {config_name:>15}: {result.backend.value} "
...
def demonstrate_routing_explanation():
    ...
    try:
        ...
        print(f"Basic Decision: {decision.recommended_backend.value} "
              f"(confidence: {decision.confidence_score:.1%})")
...
def run_performance_comparison():
    ...
    for circuit_name, circuit in circuits.items():
        ...
        try:
            ...
            print(f"  Basic Router: {basic_decision.recommended_backend.value} "
        ...
        print(f"  Enhanced Router: {enhanced_decision.recommended_backend.value} "
