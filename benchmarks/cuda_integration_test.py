    ...
    for circuit_name, circuit in circuits.items():
        ...
        for shots in shot_counts:
            ...
            try:
                ...
                backend_used = result.backend_used.value if hasattr(result.backend_used, 'value') else str(result.backend_used)
                ...
                if not is_clifford and circuit.num_qubits >= 16 and backend_used == "cuda":
                    print(f"    ✓ CUDA correctly selected for large non-Clifford circuit")
                elif is_clifford and backend_used in ["qiskit", "stim"]:
                    print(f"    ✓ Appropriate backend selected for Clifford circuit")
                elif not is_clifford and circuit.num_qubits < 16 and backend_used != "cuda":
                    print(f"    ✓ Appropriate backend selected for small non-Clifford circuit")
...
def main():
    ...
    large_non_clifford_cuda = analysis['cuda_selections'].get('large_non_clifford', 0) + analysis['cuda_selections'].get('very_large_non_clifford', 0)
