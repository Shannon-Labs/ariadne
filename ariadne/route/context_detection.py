...
class WorkflowDetector:
    ...
    def _average_circuit_pattern(self, patterns: List[CircuitPattern]) -> CircuitPattern:
        ...
        common_gates = [gate for gate, count in sorted(gate_counts.items(), 
                                                      key=lambda x: x[1], reverse=True)[:5]]
        circuit_families = [family for family, count in sorted(family_counts.items(), 
...
class ContextDetector:
    ...
    def _analyze_circuit_group(self, circuits: List[QuantumCircuit]) -> CircuitPattern:
        ...
        total_two_qubit_gates = sum(
            sum(1 for inst, _, _ in circuit.data if inst.num_qubits == 2)
            for circuit in circuits
        )
    ...
    def update_performance_history(self, backend: BackendType, execution_time: float, 
