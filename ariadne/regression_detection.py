...
class PerformanceRegressionDetector:
    ...
    def _init_database(self):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            conn.execute("CREATE INDEX IF NOT EXISTS idx_metrics_backend_type ON performance_metrics(backend, metric_type)")
    ...
    def _check_for_regression(self, metric: PerformanceMetric):
        ...
        baseline = self._get_baseline(baseline_key, metric.metric_type, metric.backend, metric.circuit_hash)
    ...
    def _create_regression_alert(
        ...
        # Create description
        direction = "increased" if metric.metric_type in [
            MetricType.EXECUTION_TIME, MetricType.BACKEND_LATENCY, MetricType.ERROR_RATE
        ] else "decreased"
    ...
    def _cleanup_old_data(self):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            cursor = conn.execute(
                "DELETE FROM regression_alerts WHERE detection_timestamp < ?",
                (alert_cutoff,)
            )
    ...
    def acknowledge_alert(self, alert_id: int):
        ...
        with sqlite3.connect(self.db_path) as conn:
            conn.execute(
                "UPDATE regression_alerts SET acknowledged = TRUE WHERE id = ?",
                (alert_id,)
            )
            conn.commit()
